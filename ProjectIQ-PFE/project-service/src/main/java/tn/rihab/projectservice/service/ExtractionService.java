...
public class ExtractionService {
    ...
    private final tn.rihab.projectservice.messaging.EventPublisher eventPublisher;
    ...
    public ExtractionResponseDto extractPhase1(UUID dossierId, String documentText) {
        ...
        tn.rihab.projectservice.model.entity.Dossier dossier = dossierRepository.findById(dossierId)
            .orElseThrow(() -> new IllegalStateException("Dossier non trouvé"));

        String textToSend = documentText;
        if (Boolean.TRUE.equals(dossier.getIsPrivate())) {
            log.info("[Anonymization] Application du masque DLP sur le texte P1");
            textToSend = anonymizationService.mask(documentText);
        }
        ...
        // Démasquer si privé
        if (Boolean.TRUE.equals(dossier.getIsPrivate()) && response.getChamps() != null) {
            response.getChamps().forEach((k, v) -> {
                if (v.getValeur() != null && v.getValeur() instanceof String s) {
                    v.setValeur(anonymizationService.unmask(s));
                }
                if (v.getSource() != null) {
                    v.setSource(anonymizationService.unmask(v.getSource()));
                }
            });
        }
    ...
    public ChampResult reextractField(UUID dossierId, String fieldName, String documentText) {
        ...
        tn.rihab.projectservice.model.entity.Dossier dossier = dossierRepository.findById(dossierId).orElseThrow();
        String textToSend = documentText;
        if (Boolean.TRUE.equals(dossier.getIsPrivate())) {
            textToSend = anonymizationService.mask(documentText);
        }
        ...
        ExtractionRequestDto request = ExtractionRequestDto.builder()
                .dossierId(dossierId).documentText(textToSend).phase("REFIELD_" + fieldName).build();

        ExtractionResponseDto extractionResp = iaServiceClient.reextractField(request);
        ChampResult result = extractionResp.getChamps().get(fieldName);

        if (result != null) {
            String val = String.valueOf(result.getValeur());
            if (Boolean.TRUE.equals(dossier.getIsPrivate())) {
                val = anonymizationService.unmask(val);
                result.setValeur(val);
                if (result.getSource() != null) {
                    result.setSource(anonymizationService.unmask(result.getSource()));
                }
            }
        ...
        eventPublisher.publishIaAuditLog(dossierId, "REEXTRACTION_" + fieldName, extractionResp);
        ...
        champs.forEach((fieldName, result) -> {
            ...
            ExtractionMetadata meta = metadataRepository.findByDossierIdAndFieldName(dossierId, fieldName)
                    .orElseGet(() -> ExtractionMetadata.builder()
                            ...
                            .source("claude_extraction").build());
