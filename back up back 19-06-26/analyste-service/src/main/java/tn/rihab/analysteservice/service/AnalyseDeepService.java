...
public class AnalyseDeepService {
    ...
    public void runFullPipeline(UUID dossierId) {
        ...
        try {
            ...
            AnalyseDossier analyse = extractPhase2(dossierId, documentText);

            // 3. Extraction + évaluation des 10 risques
            analyse = extractRisks(dossierId, analyse, documentText);
            ...
            // 6. Si NO_GO → générer rapport No-Go et s'arrêter (sauf forçage)
            if ("NO_GO".equals(pwin.getDecisionAuto())) {
                noGoReportService.generate(dossierId, dossier, pwin, analyse);
                log.info("[Pipeline] No-Go confirmé pour dossier {} — rapport généré", dossierId);
                return;
                // Note : si l'utilisateur force le Go, Angular appelle POST /scoring/{id}/force-go
                // qui relance le pipeline depuis la Phase 3 (voir ScoringController)
            }
